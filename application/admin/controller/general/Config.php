...
class Config extends Backend
{
    ...
    public function appset(){
        $row=$this->model->getGroupData('appset');
        if($this->request->isPost()){
    ...
    public function wxpayset(){
        $row=$this->model->getGroupData('wxpayset');
        if($this->request->isPost()){
            $params=$this->request->request('row/a');
            if($params['apiclient_cert']){
                file_put_contents('../cert/apiclient_cert.pem',$params['apiclient_cert']);
            }
            if($params['apiclient_key']){
                file_put_contents('../cert/apiclient_key.pem',$params['apiclient_key']);
            }
            Db::startTrans();
            $res='';
            try{
        ...
        $apiclient_cert=file_get_contents('../cert/apiclient_cert.pem');
        $apiclient_key=file_get_contents('../cert/apiclient_key.pem');
        $this->view->assign('apiclient_cert',$apiclient_cert);
        $this->view->assign('apiclient_key',$apiclient_key);
        $this->view->assign('row',$row);
        return $this->view->fetch();
    }
